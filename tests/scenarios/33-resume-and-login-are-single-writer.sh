...
# at once for one account is two OAuth refreshes rotating each other's token.
...
scenario_check "the first login flow took the login lock for account a" \
...
sleep 0.5
status="$("$BATON_BIN" --lock-status "login:a" 2>&1)"; strc=$?
...
# A DIFFERENT account is not serialized behind account a: the subject is the
# account, not one global login. Serializing everything would be a worse bug
# than the one this prevents.
b_status="$("$BATON_BIN" --lock-status "login:b" 2>&1)"; b_rc=$?
scenario_check "another account's login subject is independent and free" \
  $([ "$b_rc" -eq 0 ]; echo $?)
